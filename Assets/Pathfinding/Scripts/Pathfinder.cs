        ...
        for (int i = 0; i < height; i++)
        {
            for (int j = 0; j < width; j++)
            {
                ...
                if (CheckFullTileSize)
                {
					...
					//hit = Physics.SphereCastAll(new Vector3(x, LowestPoint, y), Tilesize / 2, Vector3.up, dist);
                }
                else
                {
                    hit = Physics.SphereCastAll(new Vector3(x, HighestPoint, y), Tilesize / 16, Vector3.down, dist);
					//hit = Physics.SphereCastAll(new Vector3(x, LowestPoint, y), Tilesize / 16, Vector3.up, dist);
                }
                bool free = true;
